...
var taAddress = '0xbbb44c808b0414c826730d1f4776e8e946e76be4';
var taABI = [
      {
        "constant": false,
        "inputs": [
...
function createStash(bankName) {
	console.log('trying to create stash for '+ bankName + ' by ' + getBanks().mas);
	var txnHash = getTransactionAgent().createBankStash(bankName, getBanks()[bankName], txnNo++, {from: getBanks().mas, gas: costForStash});
...
function pledgeStash(bankName, amt, memo, redeemFlag) {
	redeemFlag |= 0;
	var txnHash = getTransactionAgent().requestPledgeRedeem(bankName, amt, memo, redeemFlag, {from: getBanks().mas, gas: costForPledge});
	return bcUtil.getTransactionReceiptMined(getWeb3(), txnHash);
}

//---using TransactionAgent to accept pledge for a stash 
function acceptPledgeStash(txnId, memo) {
	var txnHash = getTransactionAgent().acceptPledgeRedeem(txnId, memo, {from: getBanks().mas, gas: costForAcceptPledge});
	return bcUtil.getTransactionReceiptMined(getWeb3(), txnHash);
}

//---using TransactionAgent to transfer between stashes 
function initTransfer(fromBank, toBank, amt, memo) {
	var txnHash = getTransactionAgent().initTransfer(fromBank, toBank, amt, memo, {from: getBanks().mas, gas: costForTransfer});
	return bcUtil.getTransactionReceiptMined(getWeb3(), txnHash);
}
